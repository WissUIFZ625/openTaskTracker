...
function login($email, $input_password, $mysqlcon)
{
	if($email == 'Admin'){
		
		if ($stmt = $mysqlcon->prepare("SELECT id, username, password, salt, deactivatedon FROM members WHERE  username = :EMAIL AND isBatman = 1"))
		{
			...
			if($dbresult !== false)
			{
				...
				{
					if (($dbresult['password'] == $password) )//if (($dbresult['password'] == $password) AND ((is_null($dbresult['deactivatedon'])) OR (empty($dbresult['deactivatedon']))))
					{
						$user_browser = $_SERVER['HTTP_USER_AGENT'];
